...
void SpoofedConvar::Spoof() {
	if (!IsSpoofed() && m_pOriginalCVar) {
		...
		//TODO: We dont need this call. Remove it.
		ConVar* pNewConvar = new ConVar(m_szOriginalName, m_szOriginalValue, m_iOriginalFlags, "cvar");
		g_CVar->RegisterConCommand(pNewConvar);
