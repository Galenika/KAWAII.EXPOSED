...
bool C_AutoWall::PenetrateWall(C_BasePlayer* pBaseEntity, C_BaseCombatWeapon* pWeapon, const Vector& vecPoint, float& flDamage, int& iHitbox)
{
	...
	Math::AngleVectors2(qAngles, BulletData.direction);
	Math::NormalizeAnglesVector(BulletData.direction);

	if (!SimulateFireBullet(pBaseEntity, pWeapon, BulletData))
		return false;

	flDamage = BulletData.current_damage;
	iHitbox = BulletData.enter_trace.hitgroup;
